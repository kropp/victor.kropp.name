...
title: Книги
---
<p>Здесь я записываю те книги, которые прочитал. Записи я веду скорее для себя, но если они ещё кому-нибудь пригодятся — буду рад. Из многих я выписываю интересные отрывки и цитаты, которые можно почитать по ссылкам на отдельных страницах книг.</p>
...
<script type="text/javascript">
  ...
  function updateSelections(books) {
    ...
    months.attr("id", function(d, i) { return monthNames[+d.key] + "" + this.parentNode.__data__.key; });
    ...
    var title = entry.append("h6");
    title.filter(function(d) { return d.href !== undefined; }).append("a").attr("href", function(d) { return d.href; }).text(function(d) { return d.title; });
    title.filter(function(d) { return d.href === undefined; }).text(function(d) { return d.title; });

    title.append("span").text("\u00a0").append("i").attr("class", function(d) { 
  ...
  function createYearsNavigation(books) {
    ...
    year.append("span").attr("class", "count").text(function(d, i) { return d3.sum(d.values, function(m) { return d3.sum(m.values, function(a) { return a.values.length; });  }); });
    ...
    month.append("a").attr("href", function(d, i) { return "#" + monthNames[+d.key] + "" + this.parentNode.parentNode.__data__.key; })
      .text(function(d, i) { return monthNamesRu[+d.key]; });
    month.append("span").attr("class", "count").text(function(d, i) { return d3.sum(d.values, function(a) { return a.values.length; }); });
  }

  d3.json("books.json", function(data) {
    //$("#menu-books").append("<ul class='nav nav-stacked nav-inner open' id='nav-years'></ul>");
